            ...
            <table class="table table-bordered table-responsive table-striped">
                <thead>
                   ...
                   <tr class="table-filter-custom">
                        ...
                        <th><input type="text" placeholder="Nhập tên người dùng" name="name"></th>
                    ...
                    @if (count($entry_list) > 0)
                        @foreach ($entry_list as $key => $entryData)
                            <tr>
                                ...
                                <td><a href="{{ route('user.detail', ['id' => Illuminate\Support\Facades\Crypt::encrypt($entryData['id'])]) }}">{{ $entryData['name'] }}</a></td>
                                <td>{{ $entryData['email'] }}</td>
                                <td>{{ App\Enums\Picklist::getPicklistValue('user', 'role', $entryData['role']) }}</td>
