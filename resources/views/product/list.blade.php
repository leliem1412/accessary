...
@section('content')
    <div class="breakcrumb-container">
        ...
        <div class="btn-breakcrumb-box">
            <a href="{{ route('product.create') }}" class="btn btn-primary pull-right">Thêm mới</a>
        </div>
   </div>
            ...
            <table class="table table-bordered table-responsive table-striped">
                <thead>
                    ...
                    <tr class="table-filter-custom">
                        ...
                        <th><input type="text" placeholder="Nhập mã sản phẩm" name="product_code"></th>
                        <th><input type="text" placeholder="Nhập tên sản phẩm" name="product_name"></th>
                        <th><input type="number" placeholder="Nhập số lượng" disabled name="quantity"></th>
                        <th><input type="number" placeholder="Nhập giá" name="price"></th>
                        <th>{!! App\Enums\Picklist::getPicklistView('product', 'Chọn loại sản phẩm', 'product_category') !!}</th>
                    </tr>
                    ...
                    @if (count($entry_data) > 0)
                        @foreach ($entry_data as $key => $item)
                            <tr>
                                ...
                                <td><a href="{{ route('product.detail', ['id' => Illuminate\Support\Facades\Crypt::encrypt($item['id'])]) }}">{{ $item['product_code'] }}</a></td>
                                <td>{{ $item['product_name'] }}</td>
                                <td>{{ number_format($item['quantity']) }}</td>
                                <td>{{ number_format($item['price']) }}</td>
                                <td>{{ App\Enums\Picklist::getPicklistValue('product', 'product_category', $item['product_category']) }}</td>
                                <!-- <td>{{ $item['status'] }}</td> -->
                            </tr>
                        @endforeach
                    @else
    ...
    const Product_ListView_Js = new class {
        ...
        handleSearchTableEvent() {
            $('.table-filter-search-btn').on('click', (e) => {
                ...
                const product_code = table.find('input[name="product_code"]').val();
                ...
                const product_category = table.find('select[name="product_category"]').val();
