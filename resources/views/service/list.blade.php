...
@section('content')
    <div class="breakcrumb-container">
        ...
        <div class="btn-breakcrumb-box">
            <a href="{{ route('service.create') }}" class="btn btn-primary pull-right">Thêm mới</a>
        </div>
    </div>
            ...
            <table class="table table-bordered table-responsive table-striped">
                <thead>
                    ...
                    <tr class="table-filter-custom">
                        ...
                        <th><input type="text" placeholder="Nhập mã dịch vụ" name="service_code"></th>
                        <th><input type="text" placeholder="Nhập tên dịch vụ" name="service_name"></th>
                        <th><input type="number" placeholder="Nhập giá dịch vụ" name="price"></th>
                        <th>{!! App\Enums\Picklist::getPicklistView('service', 'Chọn loại dịch vụ', 'service_category') !!}</th>
                    </tr>
                </thead>
                    ...
                    @if (count($entry_data) > 0)
                        @foreach ($entry_data as $key => $item)
                            <tr>
                                ...
                                <td><a href="{{ route('service.detail', Illuminate\Support\Facades\Crypt::encrypt($item['id'])) }}">{{ $item['service_code'] }}</a></td>
                                <td>{{ $item['service_name'] }}</td>
                                <td>{{ number_format($item['price']) }}</td>
                                <td>{{ App\Enums\Picklist::getPicklistValue('service', 'service_category', $item['service_category']) }}</td>
                                <!-- <td>{{ $item['status'] }}</td> -->
                            </tr>
                        @endforeach
    ...
    const Product_ListView_Js = new class {
        ...
        handleSearchTableEvent() {
            $('.table-filter-search-btn').on('click', (e) => {
                ...
                const service_code = table.find('input[name="service_code"]').val();
                const service_name = table.find('input[name="service_name"]').val();
                const quantity = table.find('input[name="quantity"]').val();
                const price = table.find('input[name="price"]').val();
                const service_category = table.find('select[name="service_category"]').val();
                const queryParams = {
